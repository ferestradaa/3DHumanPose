...
class pose3d(): 
    ...
    def getImageLandMarks(self, img): 
        ...
        if results.pose_landmarks: 
            ...
            for i, landmark in enumerate(results.pose_landmarks.landmark): 
                ...
                self.image_detected_landmakrs[landmark_name] = (landmark.x, landmark.y)
            cv2.imshow("detected", self.img_rgb)
            self.img_landmarks_arr = np.array(list(self.image_detected_landmakrs.values()))

            return self.image_detected_landmakrs 
        return None
    
        
    def getWordLandMark(self, img): 
        results = self.pose.process(img)
        ...
        if results.pose_landmarks: 
            ...
            #print(self.world_landmarks_arr)
            #print(self.world_landmarks_arr)
            #self.getGlobalPose()
    ...
    def getGlobalPose(self):
        ...
        world_points_hom = np.concatenate((self.world_landmarks_arr, np.ones((21,1))), axis = 1)
    ...
    def visualize3Dpose(self, detected_landmarks): 
       ...
       #self.getGlobalPose()
        ...
        #coordinates = list(detected_landmarks.values())
        
        for part in self.full_body: 
            for item in part: 
                self.ax.plot([x[item[0]], x[item[1]]], [y[item[0]], y[item[1]]], [z[item[0]], z[item[1]]])

        self.ax.set_xlim(-1, 1)  # Limites para el eje X 
        self.ax.set_ylim(-1, 1)  # Limites para el eje Y
        self.ax.set_zlim(-1, 1)  # Limites para el eje Z
